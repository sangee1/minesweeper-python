...
class Cell:
        ...
        if self.is_mine:
            ...
            if self.mines_count == 0:
                for c in self.surrounded_cells:
                    c.show_cell()
            self.show_cell()
            # if mine count == cells left you've won
            if Cell.cell_count == settings.MINES_COUNT:
                ctypes.windll.user32.MessageBoxW(
                    0, 'You Won!!!', 'Game Over', 0)

        # cancel all events if cell is opened
    ...
    def show_cell(self):
        if not self.is_open:
            Cell.cell_count -= 1
            self.cell_btn_object.configure(text=self.mines_count)
            if Cell.cell_count_label_object:
                Cell.cell_count_label_object.configure(
                    text=f"Cells left : {Cell.cell_count}")
            # if a mine candidate reset bg color
